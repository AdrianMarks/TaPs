...
class AppLocker: UIViewController {
  ...
  fileprivate var savedPin: String? {
    get {
      return KeychainWrapper.standard.string(forKey: ALConstants.kPincode)
    }
    set {
      KeychainWrapper.standard.set(newValue!, forKey: ALConstants.kPincode)
    }
  }
  ...
  fileprivate var mode: ALMode? {
    didSet {
      ...
      case .deactive:
        submessageLabel.text = "Enter your passcode to deactivate" // Your submessage for deactive mode
      case .validate:
  ...
  fileprivate func removePin() {
    KeychainWrapper.standard.removeObject(forKey: ALConstants.kPincode)
